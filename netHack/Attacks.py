...
def ARPpoisonning():
    ...
    cmd3 = 'gnome-terminal -- bash -c "docker run --rm -h "--wireshark--" --privileged --cap-add=NET_ADMIN --name Sniffing --net=host tcpdump -i pfa -w - | wireshark -k -i - "'
...
def SYNflood():
    ...
    # Wireshark to visualize the traffic
    cmd3 = 'gnome-terminal -- bash -c "docker run --rm -h "--wireshark--" --user root --privileged --cap-add=NET_ADMIN --name Sniffing --net=host tcpdump -i pfa -w - | wireshark -k -i -"'
...
def HTTPflood():
    # Victim(192.168.2.10) 
    cmd1 = 'gnome-terminal -- bash -c "docker run -it --rm -h "--Victim--" --name Victim --network=NET --ip 192.168.2.10 httpflood_victim bash"'

    # Attacker(192.168.2.20)
    cmd2 = 'gnome-terminal -- bash -c "docker run -it --rm -h "--Attacker--" --name Attacker --network=NET --ip 192.168.2.20 httpflood_attacker bash"'
...
#==============================================SMURF ATTACK==========================================
def Smurf():
    # Victim1(192.168.2.10) 
    cmd1 = 'gnome-terminal -- bash -c "docker run -it --rm -h "--Victim1--" --name Victim1 --network=NET --ip 192.168.2.10 smurf_victim1 bash"'
    
    # Victim2(192.168.2.30) 
    cmd4 = 'gnome-terminal -- bash -c "docker run -it --rm -h "--Victim2--" --name Victim2 --network=NET --ip 192.168.2.30 smurf_victim2 bash"'
...
def TCPhijacking() :
	...
	#Wireshark to visualize the traffic
	cmd4=' gnome-terminal -- bash -c " docker run --rm -h "--wireshark--" --privileged --cap-add=NET_ADMIN --name Sniffing --net=host tcpdump -i pfa -w - | wireshark -k -i - " '
...
def Scan() :
	#Target Server
	cmd1=' gnome-terminal -- bash -c " docker run -it --rm -h "--Server--" --name Server --network=NET --ip 192.168.2.15 scan_server bash" '
...
def MACflood():
	# Victim
	victim_cmd = 'gnome-terminal -- bash -c "docker run --rm -h "--Victim--" --name Victime --network=NET --ip 192.168.2.10 -it macflood_victim  bash"'
	subprocess.Popen(victim_cmd, shell=True)
	# Attacker
	attacker_cmd = 'gnome-terminal -- bash -c "docker run --rm -h "--Attacker--" --name Attacker --network=NET --ip 192.168.2.20 -it macflood_attacker  bash"'
	subprocess.Popen(attacker_cmd, shell=True)

	# Sniffing
	sniffing_cmd = 'gnome-terminal -- bash -c "docker run --rm -h "--Sniffing--" --name Sniffing --net=host tcpdump -i pfa -w - | wireshark -k -i -"'
	subprocess.Popen(sniffing_cmd, shell=True)
...
def PingOfDeath():
	# Victim
	victim_cmd = 'gnome-terminal -- bash -c "docker run --rm -h "--Victime--" --name Victime --network=NET --ip 192.168.2.10 -it pod_victim  bash"'
	subprocess.Popen(victim_cmd, shell=True)

	# Attacker
	attacker_cmd = 'gnome-terminal -- bash -c "docker run --rm -h "--Attacker--" --name Attacker --network=NET --ip 192.168.2.20 -it pod_attacker  bash"'
	subprocess.Popen(attacker_cmd, shell=True)
